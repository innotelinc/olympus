...
Generated apps are saved per signed-in identity, so a reload or a rebuild no longer loses them. In the stack the library is the `studio-data` volume (`STUDIO_DATA_DIR` points at it). Whose library is a tenancy question: by default it follows the OIDC subject, and with `CONTROL_PLANE_INTERNAL_URL` + `CONTROL_INTERNAL_TOKEN` set it follows the control-plane user id instead — and each turn is then spent on that user's **own** gateway key, with a quota check before it and usage recorded after it, rather than on the one shared `OMNIROUTE_API_KEY`. An existing subject-keyed library is adopted in place, so enabling tenancy does not orphan it. See [web/studio/README.md](web/studio/README.md#tenancy--whose-key-pays).
...
runs the container on its port. The plan also carries one field that decides where
the app's state lives: `target` is `container` by default and `convex` when the
planner chose the self-hosted Convex Atlas runs, in which case packaging points the
client at `CONVEX_URL` and deploys the functions with a deployment-scoped key. The image that Dockerfile builds is the base image for
the plan's language plus a C toolchain, added only where the plan installs something —
the tree is resolved from the project's own manifest, never from a `package-lock.json`,
because this script does not write one and `npm install` trusts one completely; a
manifest may name something with no prebuilt binary for musl, and without a compiler
that is an image that cannot be built rather than a slow one. A planning turn that
produces no usable plan stops the run, because the alternative is minutes of agent time against a stack nobody chose —
...
| Gateway | `OMNIROUTE_BASE_URL`, `OMNIROUTE_API_KEY` | **The gateway is a shared platform service and this stack starts none** — the single OmniRoute runs in Group 2 (`2-voice/`, mesh `10.10.2.1`), so one provider pool serves every platform. Default: `http://10.10.2.1:20128/v1` — OmniRoute serves its OpenAI-compatible API and its dashboard on that one port. (This stack's Authentik SSO proxy adds a `:20129` door, which is the loopback-published deployment's answer to "the dashboard still requires Authentik"; over the mesh the direct address is the default.) Gateway published on *this* host's loopback? the things that talk to it — Studio, the SSO proxy — need host networking via `compose.host-gateway.yml` (`make docker-studio-up`), and `OMNIROUTE_BASE_URL=http://127.0.0.1:20128/v1`. Not the published name: `/v1` is refused there by edge rule (`make gateway-edge --deny-path /v1`), because this gateway does not validate the API key and the public name would otherwise hand out unauthenticated inference. See [docs/gateway-sso.md](docs/gateway-sso.md) |
...
| [docs/host-migration.md](docs/host-migration.md) | Consolidating Olympus onto `192.168.1.46` — the two-host split that made every sign-in fail with `invalid_client` while the local `.env` was correct, what still lives on `.10`, and `scripts/migrate-studio-data.py` for Studio's volume |
| `scripts/verify-sso.py` | The Studio sign-in test — the client credentials at the token endpoint, the edge's forward target, anonymous refusal, and a full authorization-code flow that must open the app |
