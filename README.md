...
| Gateway dashboard | `make gateway-oidc`, `gateway-sso-up`, `gateway-edge`, `gateway-edge-check` | Puts the OmniRoute dashboard behind Authentik via an identity-aware proxy, restricted to a group, and publishes it — DNS record, certificate and NPM host — through Cerulean. The check walks the public name link by link and names the first one that is broken, because the report that arrives is "it's not resolving" and that is almost never what happened. See [docs/gateway-sso.md](docs/gateway-sso.md) |
