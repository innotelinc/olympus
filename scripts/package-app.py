...
WORKDIR /app
# `npm ci` installs the locked tree whether or not it is complete, so a dependency
# without a prebuilt binary for musl has to be compiled here — and this image carries
# no compiler. Ahead of the manifests so it stays cached when only the source changes.
# The runtime stage below is a fresh image and keeps none of it, which is what stops a
# toolchain nobody runs from reaching the published image.
...
# The fields of `package-lock.json`'s root entry that tie it to a `package.json`. npm
# writes all of them for the package it installed, so a lockfile without them — or
# with different ones — was built from a manifest that is not the one above.
ROOT_PACKAGE_FIELDS = ("name", "version", "dependencies", "devDependencies")


def lockfile_is_ours(app_dir: Path) -> bool:
    """Whether `package-lock.json` is one npm built from *this* project's manifest.

    npm trusts a lockfile completely. `npm ci` refuses to run when the lock and
    `package.json` disagree, and when they agree it installs the locked tree whether
    or not that tree is complete — so a lockfile the packager did not write is either
    a stop or, worse, a partial install inside the image. The scaffold owns
    `package.json` byte-for-byte, which makes the test exact rather than a guess: the
    lock's root package names the same dependencies at the same versions, or it is
    not ours and `npm install` regenerates it from the manifest.
    """
    try:
    ...
    # `[]` is valid JSON with neither of these attributes, so the types are checked
    # rather than assumed: a file that is not a lockfile is simply not ours.
    packages = lock.get("packages") if isinstance(lock, dict) else None
    root = packages.get("") if isinstance(packages, dict) else None
    if not isinstance(root, dict):
        return False

    ours = json.loads(PACKAGE_JSON)
    return all(root.get(field) == ours.get(field) for field in ROOT_PACKAGE_FIELDS)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        note(
            "install:  "
            + (
                "npm ci (lockfile built from this package.json)"
                if lockfile_is_ours(app_dir)
                else "npm install (no lockfile this project wrote)"
            )
        )
    ...
    npm = which_npm()
    locked = lockfile_is_ours(app_dir)
    if (app_dir / "package-lock.json").exists() and not locked:
        note(
            "package-lock.json was not built from this project's package.json — "
            "regenerating it, because npm installs whatever a lockfile says"
        )
