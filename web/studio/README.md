...
source files and Studio parses them as they stream. Iterate in plain language — the
current files are sent with each new instruction, framed as an addition rather than
a rewrite, because a build is developed over many turns and not produced in one.

## Two kinds — and the stack is not one of them

| | **App** | **Website** |
|---|---|---|
| It is | Software that runs on a server and keeps state | Static content served over HTTP |
| Its stack | Whatever the plan proposes for the request | Whatever the plan proposes for the request |
| Finished when | It is built **and running as its own container** | It is built into its output directory |
| Delivered as | A name in front of that container, or a zip | A name serving the output, or a zip |

The kind decides one thing: whether the result keeps state. It does **not** decide
the language, the framework or the database. Those come from the plan.

## The plan: proposed, then confirmed

Nothing is generated until a stack has been agreed. Asking Studio to build runs a
**planning turn** first (`/api/plan`), and the answer is shown before any code is
written:

- the name, and the hostname it will publish under (derived from the name, not
  chosen by the model — it becomes a network name)
- the stack: language, frameworks, datastore or none
- the commands that will actually run it — `install`, `build`, `start` — and the
  port the app listens on **inside its container**
- the files the model intends to write
- anything the plan thinks is worth flagging

Confirm it and the code is written against it. Change the prompt behind a plan and
it is shown as out of date rather than silently confirmed, because a plan answers
one instruction.

Why it is a separate step rather than a guess: Studio used to hardcode the stack —
React plus a generated Node/SQLite server for an app, React for a website — which
meant a request for a Flask app, an Astro site or anything else got the closest of
the two. The plan is what replaced that. The trade is deliberate: the project is
now wholly the model's to write, so the guarantee that used to come from a
generated scaffold now comes from the runner installing, building and starting
exactly what the plan declared, and reporting the real error when it does not.

See [docs/site-publishing.md](../../docs/site-publishing.md).
...
```
browser ──POST /api/plan────▶ route handler ──▶ OmniRoute /chat/completions
   ▲                    │                            (one JSON plan, no stream)
   │◀───────────────────┘  shown for confirmation
   │
   │  confirm
   ▼
browser ──POST /api/generate──▶ route handler ──▶ OmniRoute /chat/completions
   ▲                     (the confirmed plan travels with the request)
   │                                                        │
   │◀──────── plain-text stream of source ◀── SSE frame ─────┘
   │
   ├─ parseFiles()   extract <file path="…"> blocks as they arrive, live
   ├─ mergeFiles()   fold them into the project: unmentioned files are kept
   └─ CodeView       show them, and the progress, while they are still arriving

browser ──POST /api/projects/<id>/build──▶ build-queue file
                                                   │
...
| Gateway config, SSE parsing, model choice | `lib/omniroute.ts` |
...
The generation turn requires the model to answer with nothing but file blocks
(`generationMessages` in `lib/plan.ts`):
...
- On a first build, **every** file the project needs — nothing exists yet and
  there is no scaffold around it, so the manifest (package.json,
  requirements.txt, go.mod, …) is the model's to write too.
- On a development turn, only the files that change. A file the turn does not
  mention is kept as it is (`mergeFiles`), which is what makes an addition
  additive instead of a rewrite — and what stops a twenty-file project being
  re-sent to change one file and truncated on the last one.
- Files are rewritten in full each turn — never patches.
- The server listens on the plan's port and binds every interface. A server on
  `127.0.0.1` is unreachable from outside its container, and that failure is
  invisible from inside.

Deletion is deliberately not expressible. There is no way to tell "I forgot this
file" from "I meant to remove it", and guessing wrong loses work the user cannot
