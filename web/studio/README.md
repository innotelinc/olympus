...
## Two kinds, and they are two products

| | **App** | **Website** |
|---|---|---|
| The model writes | `src/App.tsx`, `server/schema.sql` | `src/App.tsx` (+ components, styles) |
| It is | A full-stack application: React client, its own API, SQLite | Static React, no server and no data |
| Finished when | It is packaged **and running as its own container** | It is packaged into `dist/` |
| Delivered as | A name in front of that container, or a zip | A name serving `dist/`, or a zip |

The kind is not a preference — it decides the system prompt, what a build has to
produce before it counts as built, and what **Export It** and **Download It** do.
An app is finished nowhere near the point a website is: `server/main.ts` (the API)
and the `Dockerfile` are generated by `scripts/package-app.py` from
`server/schema.sql`, so the model writes the data model and the interface and never
the request path. See [docs/site-publishing.md](../../docs/site-publishing.md).
...
There is no live preview, and that is a consequence of the two kinds rather than a
missing feature: an app's client needs a build *and* a server before it renders, and
a website's needs a build. What the frame would show is a blank page.
...
| Piece | File |
| --- | --- |
| Workspace UI (prompt, stream, tabs, the five deliveries) | `components/Studio.tsx` |
